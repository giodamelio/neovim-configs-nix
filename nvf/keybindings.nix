...
in {
  config.vim = {
    ...
    luaConfigRC = {
      # Treefmt commands and keybinding
      treefmt = ''
        if vim.fn.executable("treefmt") == 1 then
          ...
          end, { nargs = "*", desc = "Format file with treefmt", complete = function(arg_lead) return vim.fn.getcompletion(arg_lead, "file") end })
      ...
      user-commands = ''
        ...
        vim.api.nvim_create_user_command("LspCapabilities", function()
          ...
          Snacks.win({ text = table.concat(content_lines, "\n"), ft = "markdown", title = "LSP Capabilities", wo = { wrap = true } })
      ...
      # NixInstall command and notification for nixos-configs
      nix-install = ''
