...
vim.keymap.set('n', '<leader><leader><Tab>p', '<cmd>Grapple cycle_tags prev<cr>', { desc = 'Grapple cycle previous tag' })

-- Diagnostics and Trouble.nvim
vim.keymap.set('n', '<leader>dd', '<cmd>Trouble diagnostics toggle filter.buf=0<cr>', { desc = 'Trouble document diagnostics' })
...
vim.keymap.set('n', '<leader>lS', snacks.picker.workspace_lsp_symbols, { desc = 'Show workspace symbols' })
...
vim.api.nvim_create_autocmd('FileType', {
  pattern = 'lua',
  callback = function()
    vim.keymap.set({ 'n', 'v' }, '<localleader>e', '<cmd>LuaEval<cr>', { buffer = true, desc = 'Evaluate current file/selection' })
  end,
})
